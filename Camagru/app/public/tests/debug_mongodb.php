...
$dsn = EnvLoader::get('MONGODB_URL',);
echo "<h1>MongoDB dsn ---b {$dsn} a---</h1>";
$dsn = EnvLoader::get('MONGODB_URL', 'mongodb://mongodb:27017');
$username = EnvLoader::get('MONGO_INITDB_ROOT_USERNAME', 'admin');
$password = EnvLoader::get('MONGO_INITDB_ROOT_PASSWORD', 'admin123');
$database = EnvLoader::get('MONGO_INITDB_DATABASE', 'camagru');
echo "<h1>MongoDB Debugging Script</h1>";
try {
...
if (extension_loaded('mongodb')) {  // Check for 'mongodb' extension
...
try {
  ...
  $user_init = EnvLoader::get('MONGO_INITDB_ROOT_USERNAME', 'admin');
  $pass_init = EnvLoader::get('MONGO_INITDB_ROOT_PASSWORD', 'admin123');
  $dsn = EnvLoader::get('MONGODB_URL', "mongodb://$user_init:$pass_init@mongodb:27017");
  $port = EnvLoader::get('MONGODB_PORT', '27017');
  $connectionStrings = [
    $dsn,     // Your actual docker-compose setup
    "mongodb://localhost:$port",   // If running locally
    "mongodb://$user_init:$pass_init@mongodb:$port"  // Alternative auth
  ];
